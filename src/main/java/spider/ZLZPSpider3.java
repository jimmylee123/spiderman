...
public class ZLZPSpider3 {
	private static Logger logger = LoggerFactory.getLogger(ZLZPSpider3.class); // 日志记录

	public static void main(String[] args) {
		 File f = new File("C:\\lihaixiang\\zlzp\\qg\\zlzp.txt");
      	try {
      		if(!f.exists()){
				f.createNewFile();
      		}
      		BufferedWriter bw =  new BufferedWriter(new FileWriter(f));
			JSONObject resultObject =  get("	");
